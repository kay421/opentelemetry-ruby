...
require_relative '../../../../lib/opentelemetry/instrumentation/sequel'
require_relative '../../../../lib/opentelemetry/instrumentation/sequel/patches/connection'
...
# 2. Bundle install
# - docker-compose run ex-instrumentation-sequel-test bundle install
# 3. Run test suite
# - docker-compose run ex-instrumentation-sequel-test bundle exec rake test
describe OpenTelemetry::Instrumentation::Sequel::Instrumentation do
  ...
  describe 'tracing' do
    ...
    it 'after error' do
      ...
      end.must_raise Sequel::UndefinedColumn
      ...
      _(span.events.first.attributes['exception.type']).must_equal 'Sequel::UndefinedColumn'
    ...
    it 'uses database name as span.name fallback with invalid sql' do
      ...
      end.must_raise Sequel::SyntaxError
      ...
      _(span.events.first.name).must_equal 'exception'
      _(span.events.first.attributes['exception.type']).must_equal 'Sequel::SyntaxError'
      assert(!span.events.first.attributes['exception.message'].nil?)
      assert(!span.events.first.attributes['exception.stacktrace'].nil?)
    end
    ...
    describe 'when db_statement is obfuscate' do
      ...
      it 'obfuscates SQL parameters in db.statement' do
        ...
        expect do
          client.exec(sql)
        end.must_raise Sequel::UndefinedTable
